...
- **AI Processing**: Google Gemini 2.0 Flash 모델을 활용한 3줄 핵심 요약 및 비즈니스 인사이트 추출
- **Communication Automation**: 슬랙 웹훅(Webhook)을 통한 즉각적인 리포트 발행
- **Serverless Scheduling**: GitHub Actions를 활용하여 매일 아침 9시(KST) 완전 자동 실행 (Zero-Maintenance)
...
GitHub 저장소의 `Settings > Secrets and variables > Actions`에 아래 항목을 등록해야 합니다.
- `NAVER_CLIENT_ID`: 네이버 개발자 센터 발급 아이디
- `NAVER_CLIENT_SECRET`: 네이버 개발자 센터 발급 시크릿
